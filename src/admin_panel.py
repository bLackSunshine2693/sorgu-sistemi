...
def init_db():
    ...
    conn.execute("""CREATE TABLE IF NOT EXISTS licenses(
        ...
        machine_id TEXT DEFAULT '',usb_serial TEXT DEFAULT '',
...
def api_create_device(cid):
    ...
    if not machine_id and not usb_serial:
        conn.close();return jsonify({"error":"Makine ID veya USB Serial girilmelidir"}),400
    if not machine_id: machine_id = "USB_ANY"  # Tüm makinelerde çalışır
...
<head>
    ...
    <style>
        ...
        #panelContainer { transition: transform 0.3s cubic-bezier(0.4,0,0.2,1); }
    </style>
</head>
...
<!-- ANA İÇERİK -->
<div id="mainApp" class="hidden p-4 md:p-6 w-full overflow-x-hidden">
<div class="w-full space-y-6">

    <!-- 🔔 SÜRESİ YAKLAŞAN LİSANSLAR UYARI MERKEZİ -->
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
        ...
        <div class="bg-[#111a2e] border border-slate-800/80 rounded-lg p-6 text-center shadow-lg">
            <div id="expiredCountCard" class="text-3xl font-bold text-red-500 mb-1">—</div>
    ...
    <div class="bg-[#111a2e] border border-slate-800/80 rounded-xl p-6 shadow-xl">
        <div class="flex justify-between items-center mb-6 flex-wrap gap-2">
            ...
            <button onclick="fillMyMid()" class="bg-[#1a263e] hover:bg-slate-700 text-xs text-slate-300 px-4 py-2 rounded border border-slate-700 transition">
                Bu Bilgisayarın MAC ID'sini Al
            </button>
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <div>
                <label class="block text-[11px] font-semibold text-slate-400 mb-1.5 uppercase tracking-wider">MAC ID *</label>
                <input id="fMid" type="text" placeholder="wmic ile alınan 32 haneli ID..." class="w-full bg-[#0b111e] border border-slate-800 rounded px-3 py-2 text-sm focus:outline-none focus:border-blue-500 text-slate-300 placeholder-slate-600 font-mono">
            </div>
            <div>
                <label class="block text-[11px] font-semibold text-slate-400 mb-1.5 uppercase tracking-wider">USB SERİ NO (USB VERSİYONU)</label>
                <input id="fUsb" type="text" placeholder="Boş bırakılabilir" class="w-full bg-[#0b111e] border border-slate-800 rounded px-3 py-2 text-sm focus:outline-none focus:border-blue-500 text-slate-300 placeholder-slate-600">
            </div>
            <div>
        ...
        <div id="formMsg" class="mt-4 hidden"></div>
        <div class="pt-4">
            <button onclick="createLicense()" class="bg-blue-600 hover:bg-blue-500 text-white text-xs font-semibold px-5 py-2.5 rounded shadow-lg transition flex items-center gap-2">
                <i class="fa-solid fa-key text-[11px]"></i> Lisans Oluştur
            </button>
        </div>
    </div>
    ...
    <div class="bg-[#111a2e] border border-slate-800/80 rounded-xl shadow-xl overflow-hidden">
        <div class="p-4 border-b border-slate-800 flex justify-between items-center bg-[#131d33]">
            <h3 class="text-sm font-semibold text-white flex items-center gap-2">📄 Tüm Lisanslar</h3>
        ...
        <div class="overflow-x-auto w-full">
            <table class="w-full text-left border-collapse min-w-[1000px]">
                ...
                <tbody id="licBody" class="text-xs text-slate-300 divide-y divide-slate-800/50">
                    <tr><td colspan="7" class="p-8 text-center text-slate-500">Yükleniyor...</td></tr>
                </tbody>
            </table>
        </div>
    </div>
...
<div id="devicePanel" class="fixed inset-0 z-50 invisible transition-all duration-300">
    ...
    <div class="absolute right-0 top-0 bottom-0 w-full max-w-xl bg-[#111a2e] border-l border-slate-800 p-6 shadow-2xl flex flex-col h-full transform translate-x-full transition-transform duration-300" id="panelContainer">
        ...
        <!-- Yeni Cihaz Formu -->
        <div class="bg-[#0b111e] border border-slate-800 rounded-lg p-4 mb-4 space-y-4 shrink-0">
            ...
            <div class="grid grid-cols-1 gap-3" id="inputContainer">
                <div id="macInputGroup">
                    ...
                    <input type="text" id="newCpuId" placeholder="Cihazın benzersiz MAC veya Donanım adresi" class="w-full bg-[#111a2e] border border-slate-800 rounded px-2.5 py-1.5 text-xs text-slate-300 focus:outline-none focus:border-blue-500 placeholder-slate-600 font-mono">
                </div>
        ...
        <div class="flex-1 overflow-y-auto space-y-2 pr-1" id="panelDeviceList"></div>
    </div>
</div>

<!-- LİSANS MODAL -->
<div id="licModal" class="fixed inset-0 z-[60] hidden items-center justify-center" style="display:none">
...
<script>
    ...
    async function loadTable() {
        ...
        if(!rows.length) {
            tb.innerHTML = '<tr><td colspan="7" class="p-8 text-center text-slate-500">Henüz kayıt yok</td></tr>';
        ...
        rows.forEach(r => {
            ...
            if(days <= 10 && r.nearest_end !== 'SINIRSIZ') {
                alertCount++;
                al.innerHTML += `<li><span class="text-amber-400 font-bold">${r.name}</span> — 
                    ${days < 0 ? `<span class="text-red-400 font-bold">${Math.abs(days)} gün önce doldu!</span>` :
                      days === 0 ? `<span class="text-red-400 font-bold">Bugün bitiyor!</span>` :
                      `Süresinin bitmesine <span class="text-red-400 font-bold">${days} gün kaldı!</span>`}
                    (Bitiş: ${r.nearest_end})</li>`;
            }

            const pcBadge = r.pc_count ? `<span class="bg-blue-500/10 text-blue-400 border border-blue-500/20 px-2 py-0.5 rounded text-[11px] font-medium">${r.pc_count} PC</span>` : '';
            const usbBadge = r.usb_count ? `<span class="bg-purple-500/10 text-purple-400 border border-purple-500/20 px-2 py-0.5 rounded text-[11px] font-medium">${r.usb_count} USB</span>` : '';
            const noBadge = !r.pc_count && !r.usb_count ? '<span class="bg-slate-800 text-slate-500 border border-slate-700/50 px-2 py-0.5 rounded text-[11px]">Cihaz Yok</span>' : '';
            ...
            tb.innerHTML += `<tr class="hover:bg-slate-800/30 transition">
                ...
                <td class="p-4 text-center space-x-2 whitespace-nowrap">
                    <button onclick="openDevicePanel(${r.id}, ${r.quota})" class="bg-blue-600/20 hover:bg-blue-600 text-blue-400 hover:text-white border border-blue-600/30 px-2.5 py-1 rounded text-[11px] transition inline-flex items-center gap-1">
    ...
    // ── ANA FORM — LİSANS OLUŞTUR ─────────────────────────────────────────────
    async function fillMyMid() {
        const d = await(await fetch('/api/machine_id')).json();
        document.getElementById('fMid').value = d.machine_id;
        if(d.usb_serial) document.getElementById('fUsb').value = d.usb_serial;
    }
    ...
    async function createLicense() {
        ...
        if(!lr.ok) { showFormMsg(ld.error||'Hata', 'red'); return; }
        ...
        showFormMsg(`✅ ${ld.license_id} oluşturuldu`, 'green');
        document.getElementById('fMid').value = '';
        document.getElementById('fUsb').value = '';
    ...
    // ── CİHAZ PANELİ ──────────────────────────────────────────────────────────
    async function openDevicePanel(cid, quota) {
        currentCid = cid; currentTotal = quota;
        setLicenseType('PC');
        document.getElementById('newDeviceStart').value = todayStr;
        document.getElementById('newDeviceEnd').value = nextYear;
    ...
    async function renderDevices() {
        ...
        document.getElementById('panelClientName').textContent = cust.name;
        ...
        document.getElementById('totalSpan').textContent = currentTotal;

        const list = document.getElementById('panelDeviceList');
        list.innerHTML = '';
        if(!lics.length) {
            list.innerHTML = '<div class="text-xs text-slate-500 text-center py-4">Kayıtlı cihaz yok</div>';
            return;
        }
        lics.forEach(dev => {
            const days = dev.days;
            ...
            card.innerHTML = `
                <div class="flex justify-between items-start">
                    <div class="space-y-1 text-[11px]">
                        <div class="flex items-center gap-2">
                            <span class="${dev.usb_serial ? 'bg-purple-500/10 text-purple-400' : 'bg-blue-500/10 text-blue-400'} border border-current/20 px-1.5 py-0.5 rounded text-[10px] font-bold">${dev.usb_serial ? 'USB' : 'PC'}</span>
                            <span class="font-mono text-slate-500 text-[10px]">${dev.license_id}</span>
                        </div>
                        ...
                        <div class="pt-0.5"><span class="${badgeClass} px-1.5 py-0.5 rounded text-[10px] font-semibold">${remainingText}</span></div>
                    </div>
                ...
                <div class="bg-[#121b2d] border border-slate-800 rounded px-2.5 py-2 flex items-center justify-between gap-2">
                    <div class="font-mono text-[11px] text-slate-400 overflow-hidden text-ellipsis whitespace-nowrap flex-1">${dev.license_id}</div>
                    <button onclick="showLic('${dev.license_file.replace(/'/g,"\\'")}','${dev.license_id}')"
    ...
    function setLicenseType(type) {
        ...
        const activeClass = "bg-blue-600 text-white border border-blue-500 py-2 rounded text-xs font-semibold transition flex items-center justify-center gap-2 shadow-md";
        const passiveClass = "bg-[#111a2e] text-slate-400 border border-slate-800 hover:bg-slate-800 hover:text-slate-200 py-2 rounded text-xs font-semibold transition flex items-center justify-center gap-2";
        if(type === 'PC') {
            btnPC.className = activeClass; btnUSB.className = passiveClass;
            inputContainer.className = "grid grid-cols-1 gap-3";
            usbGroup.classList.add('hidden');
            document.getElementById('newUsbId').value = '';
        } else {
            btnUSB.className = activeClass; btnPC.className = passiveClass;
    ...
    async function generateDeviceLicense() {
        ...
        if(!macId) { showDevMsg('Lütfen MAC ID alanını doldurunuz!'); return; }
        if(activeLicenseType === 'USB' && !usbId) { showDevMsg('USB SERİ NO alanı boş bırakılamaz!'); return; }
        ...
        if(!r.ok) { showDevMsg(d.error||'Hata'); return; }
